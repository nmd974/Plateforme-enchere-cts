<?php
    function controleImage($image_upload):string
    {
        $error = false;
        ...
        if(!in_array("." . $fileExt, $validExt))//On recherche dans le tableau des extensions valides si l'extension du fichier ajouté correspond
            { 
                $error = true;
                return '<div class="alert alert-danger">Le fichier n\'est pas une image !!</div>';
            }
        if($_FILES['image_upload']['error'] > 0)//On verifie dans la variable $_FILES s'il n'y a pas d'erreur interne
            {
                $error = true;
                return '<div class="alert alert-danger">Erreur survenue lors du transfert de l\'image</div>'; //Si oui alors on arrete la fonction et on affiche qu'il y a eu une erreur lors du transfert
            }
        ...
        if($fileSize > $maxSize) //Taille de l'image doit être < à $maxSize
            {
                $error = true;
                return '<div class="alert alert-danger"> Le fichier est trop lourd !!</div>'; //Si trop lourd alors on envoie le message que le fichier est trop lourd
            }
        if(!$error){
            ...
            $fileDir = dirname(__ROOT__)."/img/" . $idName . "." . $fileExt; //On spécifie la direction d'enregistrement de l'image
            ...
            if($resultat){
                return $image_upload['image_upload'];//On retourne le nom de l'image pour enregistrement
            }
        }
